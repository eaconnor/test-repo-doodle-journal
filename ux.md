...
| ~~Pizarro (2004) primary~~ **CLOSED — and it reversed** | The secondhand claim was contradicted: no combined art+writing condition was ever run, writing beat art on symptoms, and art won on *enjoyment and completion*. New `[R]` claim: the drawing's measured benefit is retention, not outcome. | R-08 resolved |
...
- [ ] G1-25 — **At least one named segment has primary evidence, from a person in that segment, that they have the stated problem** · traces_to: §3, R-10 · verified_by: a segment id with a participant-sourced claim — **FALSE for all three segments. S-1 has evidence the *problem* exists (SRC-002) and none that S-1 has it; S-2 has no source at all; S-3 has a disciplinary premise and primary evidence against it.**
  This is the criterion that did not exist, and it is the hole the whole framework had. The spec required every *claim* to be tagged — and you can have 100% tagged claims with zero validated segments, which is exactly the state this project shipped in. Tagging measures honesty about evidence; it does not require the evidence. A Gate 1 that cannot fail on "we do not know whose problem this is" will let a team build the right thing for the wrong people and call it validated.
